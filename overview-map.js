...
function travelOverviewArtwork(days, source = {}, options = {}) {
  ...
  ${source.illustrated ? `<g fill="none" stroke-linecap="round" stroke-linejoin="round">${(source.roads || []).map(path => `<path d="${esc(path)}" stroke="#e0dfd0" stroke-width="16"/><path d="${esc(path)}" stroke="#fffdf6" stroke-width="11"/>`).join("")}</g>` : ""}
  ${routeLayers.map((route) => `<g id="overview-route-${route.day}" transform="translate(${route.laneOffset || 0} 0)" inkscape:groupmode="layer" inkscape:label="${esc(days.find((day) => day.day === route.day)?.date || route.day)}" data-date="${esc(days.find((day) => day.day === route.day)?.date || "")}" fill="none" stroke-linecap="round" stroke-linejoin="round">${pathsFor(route).map((path) => `<path d="${esc(path)}" stroke="${esc(route.color)}" stroke-width="${source.illustrated ? 8 : 7}"/><path d="${esc(path)}" stroke="#ffffff" stroke-opacity=".22" stroke-width="2"/>`).join("")}${(route.arrows || []).map(a => `<path d="M-9 -6 2 0 -9 6" transform="translate(${a.x} ${a.y}) rotate(${a.angle})" stroke="#fffdf6" stroke-width="3"/>`).join("")}</g>`).join("")}
  ...
  <g id="overview-date-legend" inkscape:groupmode="layer" inkscape:label="透明日期图例" font-family="'Times New Roman',serif" font-size="23" font-weight="700" fill="#092653" stroke="#092653" stroke-width="0.4" stroke-linejoin="round" paint-order="stroke fill">${routeLayers.map((route, index) => { const date = days.find((day) => day.day === route.day)?.date; if (!date) return ""; const y = legend.y + Math.floor(index / (legend.columns || 1)) * legend.gap; const lx = legend.x + (index % (legend.columns || 1)) * (legend.columnGap || 0); return `<g data-date="${esc(date)}"><path d="M${lx} ${y - 7} h28" fill="none" stroke="${esc(route.color)}" stroke-width="5" stroke-linecap="round"/><text x="${lx + 41}" y="${y}">${Number(date.slice(5, 7))}/${Number(date.slice(8))}${route.label ? ` · ${esc(route.label)}` : ""}</text></g>`; }).join("")}</g>
  ${source.illustrated ? `<g fill="#557264" font-family="Arial, 'PingFang SC', sans-serif"><text x="55" y="117" font-size="23">${esc(source.subtitle)}</text>${(source.notes || []).map((note,i)=>`<text x="55" y="${1037+i*30}" font-size="23">${esc(note)}</text>`).join("")}</g>` : ""}
  </svg>`;
}
