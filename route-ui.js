...
function setupRouteExplorer() {
  ...
  const showPopover = (pin, content, map = false) => {
    ...
    popover.setAttribute("role", "dialog"); popover.setAttribute("aria-label", map ? "地点导航" : "交通信息");
  ...
  document.addEventListener("click", (event) => {
    ...
    if (option && popover) {
      ...
      $("[data-popup-address]", popover).textContent = query;
