...
export function buildIllustratedRegion(map, colors) {
  ...
  return {id:map.region.id,label:map.region.label,countryCode:map.region.countryCode,scope:'trip-illustration',mapMode:'original-illustration',templateId:'autumn-2026-original',days:routes.map(r=>r.day),canvas:{width:1100,height:1120},baseImage:art.baseImage,title:map.region.title,ariaLabel:`${map.region.label}原创手绘行程示意图`,description:map.region.description,disclaimer:map.disclaimer,heading:{text:map.region.label+' · 旅行手绘地图',x:55,y:80,size:43},subtitle:art.subtitle,legend:{x:55,y:155,gap:38,columns:1,columnGap:0},annotations:[],places,overviewPlaceIds:visible.map(p=>p.id),routes,dailyLayouts,illustrated:true,roads:edges.map(e=>path(e.points)),notes:art.notes};
}
