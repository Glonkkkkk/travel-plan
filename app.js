...
function flightStopMarkup(stop, position, journeyStartDate) {
  let timing;
  if (position === 0) {
    timing = `<span>${escapeHtml(relativeFlightDate(stop.departure.date, journeyStartDate))}</span><b>${escapeHtml(stop.departure.time)}</b><span>出发</span>`;
  } else if (position === stop.totalStops - 1) {
    timing = `<span>${escapeHtml(relativeFlightDate(stop.arrival.date, journeyStartDate))}</span><b>${escapeHtml(stop.arrival.time)}</b><span>抵达</span>`;
  } else {
...
function navigationDestinations(item) {
  if (item.navigation === false) return [];
  ...
  if (referencedPlaceIds.length) {
    return referencedPlaceIds.map((placeId) => state.data.places.find((place) => place.id === placeId)).filter(Boolean).map((place) => ({
      ...
      provider: place.navigation?.provider || "amap",
      query: place.navigation?.query || place.address || `${place.nameZh || place.name}${place.cityOrArea ? `, ${place.cityOrArea}` : ""}`,
      directUrl: Boolean(place.navigation?.url),
      url: place.navigation?.url || ""
    }));
  }
  ...
  const catalogPlaces = state.data.places
    ...
    .map((place) => ({
      id: place.id,
      label: place.nameZh || place.name,
      query: place.navigation?.query || [place.name, place.cityOrArea].filter(Boolean).join(", "),
      url: place.navigation?.url || "",
      directUrl: Boolean(place.navigation?.url),
      ...
      <div class="rental-details">
        <div class="rental-stops">
          <div class="rental-stop">
            ...
            <div><b>${formatCompactDate(rental.pickup.date)} ${escapeHtml(rental.pickup.time)}</b><span>${escapeHtml(rental.pickup.location)}</span></div>
          </div>
          <div class="rental-stop">
